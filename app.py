...
# Import functions from the kpi package
from kpi import load_dataframe, calculate_kpi, print_report
import pandas as pd
from io import StringIO

app = FastAPI(
    title="Employee KPI Calculator API",
    description="API for calculating monthly workload KPI for employees",
    version="1.0.0"
)

# Define the CSV file path - Update this path to match your CSV file location
CSV_FILE_PATH = "dataset/employee_attendance_report_20250626_085838.csv"
...
async def root():
    ...
    return {
        "message": "Employee KPI Calculator API",
        "version": "1.0.0",
        "endpoints": {
...
async def calculate_employee_kpi():
    ...
    try:
        ...
        if monthly_report_df.empty:
            return {
                "status": "success",
                "message": "No valid data found to generate KPI report",
                "employees": [],
                "total_employees": 0
            }
        ...
        return {
            ...
            "summary": {
                "highest_kpi": max([emp['monthly_kpi'] for emp in employees_data]) if employees_data else 0,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"An error occurred during KPI calculation: {str(e)}"
        )
...
async def get_employee_kpi(first_name: str, last_name: str):
    ...
    try:
        ...
        if not employee_found:
            raise HTTPException(
                status_code=404,
                detail=f"Employee '{first_name} {last_name}' not found in the dataset"
            )
